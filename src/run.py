...
def division(frame):
    ...
    blocks = [frame[:length / 2, :width / 2], frame[length / 2:, :width / 2],
              frame[:length / 2, width / 2:], frame[length / 2:, width / 2:]]
    return blocks
...
def detect_edge(img):
    ...
    contours, hierarchy = cv2.findContours(
        edge_map, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    return contours
...
def main():
    ...
    if rv1:
        ...
        while True:
            ...
            # Motion Detection
            move, dist = detect_move(cv2.cvtColor(
                im1, cv2.COLOR_BGR2GRAY), cv2.cvtColor(im2, cv2.COLOR_BGR2GRAY))
            ...
            if move:
                ...
                # Save clips into MP4
                    ...
                    save2video('%s/shot_%s' % (args.folder, num_shot),
                               buff, fps, (width, height))
                    num_shot += 1
                    buff = []
